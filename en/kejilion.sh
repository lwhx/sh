	...
	case "$action" in
		block)
			...
			if ! wget -q "$download_url" -O "${country_code,,}.zone"; then
				echo "错误：下载 $country_codeIP zone file failed"
				exit 1
			fi
		...
		allow)
			...
			# 下载 IP 区域文件
			if ! wget -q "$download_url" -O "${country_code,,}.zone"; then
				echo "Error: Download$country_code 的 IP 区域文件失败"
				exit 1
			fi
  ...
  while true; do
		  ...
		  case $sub_choice in
			  ...
			  15)
				  ...
				  send_stats "allow countries$country_code 的IP"
				  ;;
			  16)
...
certs_status() {
	...
	if [ -f "$file_path" ]; then
		...
		send_stats "域名证书申请失败"
		echo -e "${gl_hong}Notice:${gl_bai}证书申请失败，请检查以下可能原因并重试："
...
format_partition() {
	...
	# Confirm formatting
	read -e -p "确认格式化分区 /dev/$PARTITION 为 $FS_TYPE? (y/n):" CONFIRM
	if [ "$CONFIRM" != "y" ]; then
		echo "操作已取消。"
		return
	fi
...
linux_tools() {
  ...
  while true; do
	  ...
	  echo -e "basic tools"
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}1.   ${gl_bai}curl 下载工具 ${gl_huang}★${gl_bai}                   ${gl_kjlan}2.   ${gl_bai}wget download tool${gl_huang}★${gl_bai}"
...
linux_ldnmp() {
  while true; do
	...
	echo -e "${gl_huang}37.  ${gl_bai}更新LDNMP环境                     ${gl_huang}38.  ${gl_bai}Uninstall the LDNMP environment"
	echo -e "${gl_huang}------------------------"
...
EOF
		  ...
		  66)
			  ...
			  echo -e "10. Linux系统内核参数优化切换到${gl_huang}Balanced optimization mode${gl_bai}"
			  echo "------------------------------------------------"
...
while true; do
	  ...
	  echo -e "${gl_kjlan}17. ${gl_bai}设置时区到上海           ${gl_kjlan}18. ${gl_bai}Open all ports${gl_kjlan}51. ${gl_bai}custom directive"
	  echo -e "${gl_kjlan}------------------------${gl_bai}"
	  echo -e "${gl_kjlan}0.  ${gl_bai}返回主菜单"
	  echo -e "${gl_kjlan}------------------------${gl_bai}"
	  read -e -p "请输入你的选择: " sub_choice
